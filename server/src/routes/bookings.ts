...
router.get('/owner/:ownerId', async (req, res) => {
  const [rows] = await pool.query(
    `SELECT b.*, l.title AS listing_title, l.owner_id, l.owner_name
     FROM bookings b
...
router.post('/', async (req, res) => {
  ...
  const commissionRate = Number((settingsRows as any)[0]?.commission_rate ?? 0.1)
  const platformFee = Number(totalPrice) * commissionRate
  const ownerPayout = Number(totalPrice) - platformFee

  const [result] = await pool.execute(
    `INSERT INTO bookings (listing_id, user_id, check_in_date, check_out_date, total_price, guest_phone, guest_notes, payment_method, status, platform_fee, owner_payout)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      listingId,
      userId,
